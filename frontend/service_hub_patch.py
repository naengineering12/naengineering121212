...
items = [
    ...
    ('wastewater-treatment-plant-supplies-services', 'Wastewater Treatment Plant (WWTP) Supplies and Services', 'Factory', 'https://static.wixstatic.com/media/b9b6ed_22bbec6674af4fb0b309bcc93509af32~mv2.jpg/v1/fill/w_980%2Ch_735%2Cal_c%2Cq_85%2Cusm_0.66_1.00_0.01%2Cenc_avif%2Cquality_auto/b9b6ed_22bbec6674af4fb0b309af32~mv2.jpg', 'Supply of WWTP equipment including pumps, blowers, diffusers and dosing systems; treatment chemicals such as coagulants, flocculants and disinfectants; operation and maintenance support; and technical guidance for performance improvement and compliance with standards.'),
...
# Give every service detail page a clear next step after its service-specific details.
cta_marker = '<Button to="/contact" testid={`service-detail-quote-${service.slug}`}>Request a Quote</Button>'
if 'service-detail-quote-' not in source:
    source=source.replace('</ul></div></div></main></>}', f'</ul><div className="detail-actions">{cta_marker}</div></div></div></main></>}', 1)
...
print('All services now use dedicated detail pages with service-specific support points and quote CTA.')
