...
- 2026-08-21: Services page cleaned up — removed original "HVAC" and "Fire Fighting" (kept the supplies versions), removed number badges from service card images (now 13 services). Fire Fighting Equipment image updated to a multi-extinguisher photo.
- 2026-08-21: General Order Supplies page (/supplies) fully rebuilt to match the Services-page block layout: new intro + 9 categories (Hardware & Tools, Electrical, Safety & PPE, Cleaning & Janitorial, Industrial & Maintenance Consumables, Packaging, Canteen & Pantry, IT & Misc, Office & Stationery) each with image + description + bullet list, plus an "Our Commitment" section (4 cards) + CTA. Safety & PPE and Cleaning images updated to topical photos.
- 2026-08-21: Industries page (/industries) rebuilt to use the same alternating service-row block layout (11 industries with INDUSTRY/NN label + title + description). Full frontend regression test passed (all pages, nav, quote form).
- 2026-08-21: Added new "IT Services & Equipment" page (/it-services) in header + footer nav — 7 Services-style blocks grouping all 25 requested IT items (computers, PC components, printing/peripherals, networking, servers/storage/power, security & AV, software & IT services).
- 2026-08-21: Performance optimization — img() helper reduced to w=1200/q=70 (auto webp), hero background reduced to w=1600, and loading="lazy"+decoding="async" added to all content images. All pages now load in ~0.4-0.6s (well under 3s target).
- 2026-08-21: Vercel deployment readiness — fixed backend/main.py (Vercel entrypoint) to re-export the resilient quote route from server.py (was re-introducing the email-mandatory 500 bug); unignored .env files in .gitignore so deploy has env; reduced admin/status query limits. deployment_agent = PASS, production build (CI=true) passes, backend + frontend regression tests all pass.
